...
#else
	// SSOR preconditioner: Y = M^{-1} * X (sequential triangular solves, CPU only)
	double lam;
...
int CBigLinProb::PCGSolve(int flag)
{
	...
	if(n >= 1000)
	{
		...
		if(gpu)
		{
			...
			res_o = gpu->dot(Z, b);
			if(res_o==0) { gpu->downloadSolution(n, V); return 1; }
