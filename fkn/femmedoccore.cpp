...
// ---------------------------------------------------------------
// In-memory document loading (replaces OnOpenDocument)
// ---------------------------------------------------------------
BOOL CFemmeDocCore::LoadFromDocument(
	double frequency, double precision, double relax,
	int lengthUnits, int acSolver, int problemType, int coords,
	double eZo, double eRo, double eRi,
	...
	const InMemBlockLabel *blkLabels, int numBlkLabels)
{
	...
	// Material properties
	NumBlockProps = numMatProps;
	if (numMatProps > 0) {
		blockproplist = (CMaterialProp *)calloc(numMatProps, sizeof(CMaterialProp));
		for (i = 0; i < numMatProps; i++) {
			const InMemMaterialProp &src = matProps[i];
			CMaterialProp &dst = blockproplist[i];
			dst.mu_x = src.mu_x;
			dst.mu_y = src.mu_y;
			dst.H_c = src.H_c;
			...
			dst.Hdata = NULL;
			dst.slope = NULL;
			if (src.BHpoints > 0 && src.Bdata && src.Hdata_re) {
				dst.Bdata = (double *)calloc(src.BHpoints, sizeof(double));
				dst.Hdata = (CComplex *)calloc(src.BHpoints, sizeof(CComplex));
				for (j = 0; j < src.BHpoints; j++) {
					dst.Bdata[j] = src.Bdata[j];
					dst.Hdata[j].re = src.Hdata_re[j];
					dst.Hdata[j].im = 0;
				}
				dst.GetSlopes(Frequency * 2. * PI);
			}
		}
	}

	// Boundary properties
	NumLineProps = numBdryProps;
	if (numBdryProps > 0) {
		lineproplist = (CBoundaryProp *)calloc(numBdryProps, sizeof(CBoundaryProp));
		for (i = 0; i < numBdryProps; i++) {
			const InMemBoundaryProp &src = bdryProps[i];
			CBoundaryProp &dst = lineproplist[i];
			...
			dst.Mu = src.Mu;
			dst.Sig = src.Sig;
			dst.c0.re = src.c0_re; dst.c0.im = src.c0_im;
			dst.c1.re = src.c1_re; dst.c1.im = src.c1_im;
	...
	// Point properties
	NumPointProps = numPtProps;
	if (numPtProps > 0) {
		nodeproplist = (CPointProp *)calloc(numPtProps, sizeof(CPointProp));
		for (i = 0; i < numPtProps; i++) {
			nodeproplist[i].Jr = ptProps[i].Jr;
			nodeproplist[i].Ji = ptProps[i].Ji;
			nodeproplist[i].Ar = ptProps[i].Ar;
			nodeproplist[i].Ai = ptProps[i].Ai;
		}
	}

	// Circuit properties
	NumCircProps = numCircProps;
	if (numCircProps > 0) {
		// Allocate enough for original + expanded serial circuits
		circproplist = (CCircuit *)calloc(numCircProps + numBlkLabels, sizeof(CCircuit));
		for (i = 0; i < numCircProps; i++) {
			circproplist[i].dVolts_re = circProps[i].dVolts_re;
			circproplist[i].dVolts_im = circProps[i].dVolts_im;
			circproplist[i].Amps_re = circProps[i].Amps_re;
			circproplist[i].Amps_im = circProps[i].Amps_im;
			circproplist[i].CircType = circProps[i].CircType;
			circproplist[i].OrigCirc = -1;
		}
	}

	// Block labels
	NumBlockLabels = numBlkLabels;
	if (numBlkLabels > 0) {
		labellist = (CBlockLabel *)calloc(numBlkLabels, sizeof(CBlockLabel));
		for (i = 0; i < numBlkLabels; i++) {
			const InMemBlockLabel &src = blkLabels[i];
			CBlockLabel &dst = labellist[i];
			dst.x = src.x;
			dst.y = src.y;
			dst.MaxArea = src.MaxArea;
			dst.MagDir = src.MagDir;
			dst.BlockType = src.BlockType;
			dst.InCircuit = src.InCircuit;
			dst.InGroup = src.InGroup;
			dst.Turns = src.Turns;
			dst.IsExternal = src.IsExternal;
			dst.IsDefault = src.IsDefault;
			dst.MagDirFctn = NULL;
			if (src.MagDirFctn && strlen(src.MagDirFctn) > 0) {
				dst.MagDirFctn = (char *)calloc(strlen(src.MagDirFctn) + 1, sizeof(char));
				strcpy(dst.MagDirFctn, src.MagDirFctn);
			}
		}
	}

	// Process serial circuits (same logic as end of OnOpenDocument)
	if (NumCircProps > 0) {
		NumCircPropsOrig = NumCircProps;
		CCircuit ncirc;
		for (k = 0; k < NumBlockLabels; k++) {
		...
		// Convert all serial to parallel
		for (k = 0; k < NumCircProps; k++)
			if (circproplist[k].CircType == 1) circproplist[k].CircType = 0;
	} else {
		NumCircPropsOrig = 0;
...
// ---------------------------------------------------------------
// In-memory mesh loading (replaces LoadMesh)
// ---------------------------------------------------------------
BOOL CFemmeDocCore::LoadMeshFromMemory(
	const InMemMeshNode *inNodes, int numNodes,
	const InMemMeshElement *inElements, int numElements,
	const InMemMeshEdge *inEdges, int numEdges)
{
	int i, j, k, q, n0, n1;
	double c[] = {2.54, 0.1, 1., 100., 0.00254, 1.e-04};
	...
	meshnode = (CNode *)calloc(numNodes, sizeof(CNode));
	for (i = 0; i < numNodes; i++) {
		meshnode[i].x = inNodes[i].x * c[LengthUnits];
		meshnode[i].y = inNodes[i].y * c[LengthUnits];
		int bm = inNodes[i].boundaryMarker;
		if (bm > 1) meshnode[i].bc = bm - 2;
		else meshnode[i].bc = -1;
	}

	// PBC: none for in-process mode
	NumPBCs = 0;
	pbclist = NULL;
	NumAGEs = 0;
	agelist = NULL;
	...
	// Elements
	NumEls = numElements;
	meshele = (CElement *)calloc(numElements, sizeof(CElement));
	...
	for (i = 0; i < numElements; i++) {
		meshele[i].p[0] = inElements[i].p[0];
		meshele[i].p[1] = inElements[i].p[1];
		meshele[i].p[2] = inElements[i].p[2];
		meshele[i].lbl = inElements[i].label - 1;  // 1-based → 0-based
		if (meshele[i].lbl < 0) meshele[i].lbl = defaultLabel;
		if (meshele[i].lbl >= 0)
			meshele[i].blk = labellist[meshele[i].lbl].BlockType;
		else
			meshele[i].blk = 0;
	}

	// Initialize edge BCs and element permeabilities
	for (i = 0; i < NumEls; i++)
		for (j = 0; j < 3; j++) {
			meshele[i].e[j] = -1;
			meshele[i].mu1 = -1.;
	...
	// Build node-to-element connectivity for edge BC mapping
	int *nmbr = (int *)calloc(NumNodes, sizeof(int));
	for (i = 0; i < NumEls; i++)
		for (j = 0; j < 3; j++)
			nmbr[meshele[i].p[j]]++;

	int **mbr = (int **)calloc(NumNodes, sizeof(int *));
	for (i = 0; i < NumNodes; i++) {
		mbr[i] = (int *)calloc(nmbr[i], sizeof(int));
		nmbr[i] = 0;
	}
	...
	// Map edge boundary markers to element edges
	for (i = 0; i < numEdges; i++) {
		int marker = inEdges[i].marker;
		if (marker < 0) {
			j = -(marker + 2);  // decode boundary prop index
			n0 = inEdges[i].n0;
			n1 = inEdges[i].n1;
			for (q = 0; q < nmbr[n0]; q++) {
				CElement &elm = meshele[mbr[n0][q]];
				if ((elm.p[0] == n0 && elm.p[1] == n1) || (elm.p[0] == n1 && elm.p[1] == n0)) elm.e[0] = j;
				if ((elm.p[1] == n0 && elm.p[2] == n1) || (elm.p[1] == n1 && elm.p[2] == n0)) elm.e[1] = j;
				if ((elm.p[2] == n0 && elm.p[0] == n1) || (elm.p[2] == n1 && elm.p[0] == n0)) elm.e[2] = j;
			}
		}
	}
...
// ---------------------------------------------------------------
// Cuthill-McKee from in-memory edges (no .edge file dependency)
// Same algorithm as Cuthill() but reads edge data from memory.
// ---------------------------------------------------------------
BOOL CFemmeDocCore::CuthillFromMemory(const InMemMeshEdge *edges, int numEdges)
{
	int i,j,k,n0,n1,n;
	...
	ocon[0]=(int *)calloc(2*numEdges,sizeof(int));

	// first pass: count connections per node
	for(i=0;i<numEdges;i++){
		numcon[edges[i].n0]++;
		numcon[edges[i].n1]++;
	}
	...
	// sort connections in order of increasing connectivity (bubble sort)
	...
	// search for a node to start with (minimum connectivity)
	j=numcon[0]; n0=0;
	for(i=1;i<NumNodes;i++){
		if(numcon[i]<j){ j=numcon[i]; n0=i; }
		if(j==2) break;
	}
	...
	do{
		...
		// handle multiply connected problems
		if(nxtnum[newnum[n0]+1]<0){
			for(i=0;i<NumNodes;i++)
				if(newnum[i]<0){ j=numcon[i]; n0=i; i=NumNodes; }
			for(i=0;i<NumNodes;i++){
				if((newnum[i]<0) && (numcon[i]<j)){ j=numcon[i]; n0=i; }
				if(j==2) i=NumNodes;
			}
			newnum[n0]=n;
			nxtnum[n]=n0;
	...
	// apply mapping to elements
	for(i=0;i<NumEls;i++)
		for(j=0;j<3;j++)
			meshele[i].p[j]=newnum[meshele[i].p[j]];

	// sort nodes based on newnum
	for(i=0;i<NumNodes;i++)
		while(newnum[i]!=i){
			j=newnum[i];
