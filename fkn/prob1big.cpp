...
BOOL CFemmeDocCore::Static2D(CBigLinProb &L)
{
	...
	double bestRes = 1e30;   // best residual seen so far
	int stallCount = 0;      // iterations without significant improvement
...
//////// Nonlinear Part
	...
	// converge.  Use 1000*tolerance (solution changes < 0.001% per iter).
	if((res<1000.*Precision) && (Iter>0)) LinearFlag=TRUE;

	// Stall detection: if residual hasn't improved significantly
	// in 10 iterations, accept the solution (it's effectively converged).
	if(!LinearFlag && Iter>5) {
		if (res < 0.9 * bestRes) {
			bestRes = res;
			stallCount = 0;
		} else {
