...
void CBigComplexLinProb::Finalize()
{
	...
	csrD = (CComplex *)malloc(n * sizeof(CComplex));
	csrR = (int *)malloc((n+1) * sizeof(int));
	csrC = (int *)malloc(csrNnz * sizeof(int));
	csrV = (CComplex *)malloc(csrNnz * sizeof(CComplex));
...
int CBigComplexLinProb::PBCGSolveMod(int flag)
{
	...
	// TODO: complex GPU PCG path (Metal double2)
	// The GPU backend currently supports real double-precision only.
	// When complex GPU support is added, dispatch to Metal here using
	// the CSR arrays (csrD, csrR, csrC, csrV) with double2 for CComplex.
